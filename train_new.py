...
class SequenceLightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if not self.hparams.trainer.save_val_outputs:
    ...
    def _save_tensors(self, tensors, file_name, prefix='val', max_dir_size_mb=np.float('inf')):
        ...
        dir_size_mb = sum(os.path.getsize(f) for f in os.scandir(save_dir) if f.is_file()) / 2**20
        if dir_size_mb >= max_dir_size_mb:  # del old contents
